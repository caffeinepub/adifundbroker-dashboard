...
export function InternetIdentityProvider({
  ...
  // Use a ref for the authClient so it never causes re-renders or re-triggers effects
  const authClientRef = useRef<AuthClient | undefined>(undefined);
  // Store createOptions in a ref too so the effect never needs it as a dependency
  ...
  const login = useCallback(() => {
    ...
    if (
      ...
      // Already authenticated -- just trigger success path
  ...
  const clear = useCallback(() => {
    ...
    void authClient
      .logout()
      .then(() => {
        // Keep authClientRef alive -- just clear identity and reset to idle
  ...
  // Runs exactly once on mount. Uses refs to avoid stale closure issues
  // without adding them as dependencies (which would cause re-initialization).
  useEffect(() => {
    let cancelled = false;
    void (async () => {
      try {
