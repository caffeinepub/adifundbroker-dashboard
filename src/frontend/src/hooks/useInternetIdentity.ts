...
export function InternetIdentityProvider({
  ...
  // Store authClient in a ref so it never triggers effect re-runs
  const authClientRef = useRef<AuthClient | undefined>(undefined);
  // Store createOptions in a ref so it doesn't re-trigger the init effect
  ...
  const login = useCallback(() => {
    ...
    if (
      ...
      // Already authenticated - set success so redirect fires
  ...
  const clear = useCallback(() => {
    ...
    void client
      ...
      .then(() => {
        ...
        // Do NOT null out authClientRef -- keep the client alive
        setStatus("idle");
        setError(undefined);
      })
  ...
  // Runs EXACTLY ONCE on mount. No dependency array items that can change.
  useEffect(() => {
    let cancelled = false;
    void (async () => {
      try {
        ...
        if (cancelled) return;
        // Store in ref -- never triggers re-render or re-run of this effect
        ...
        if (isAuthenticated) {
          ...
          // Set "success" for returning users so LoginPage redirect fires reliably
          setStatus("success");
        } else {
          setStatus("idle");
        }
